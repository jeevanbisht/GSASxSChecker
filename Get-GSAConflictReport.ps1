...
const vendorRemediation = {
  ...
  "Zscaler":          "Microsoft has published an official coexistence guide for Zscaler and GSA (aka.ms/GSAZscaler). Coexistence is supported across 4 scenarios (GSA for Private/Internet/M365 + Zscaler for Internet/Private). Key Zscaler configuration: In Zscaler Client Connector admin portal, go to Administration > Forwarding Profile > Add Forwarding Profile, create a profile (e.g. 'ZIA Only' or 'ZIA and ZPA') using Packet Filter-Based tunnel type. Then go to App Profiles > Windows > Add Windows Policy, select that forwarding profile, and add the following to 'HOSTNAME OR IP ADDRESS BYPASS FOR VPN GATEWAY' — IPs: 150.171.15.0/24, 150.171.18.0/24, 150.171.19.0/24, 150.171.20.0/24, 13.107.232.0/24, 13.107.233.0/24, 151.206.0.0/16, 6.6.0.0/16 — FQDNs: *.globalsecureaccess.microsoft.com and tenant-specific FQDNs (e.g. <tenantid>.internet.client.globalsecureaccess.microsoft.com). If GSA handles Internet Access, add a custom bypass for *.prod.zpath.net in the GSA Internet Access profile (Entra admin center > Global Secure Access > Connect > Traffic forwarding > Internet access profile > Custom Bypass). Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-zscaler-coexistence",
  "Netskope":         "Microsoft has published an official coexistence guide for Netskope and GSA (aka.ms/GSANetskope). Coexistence is supported across 4 scenarios (GSA for Private/Internet/M365 + Netskope for Internet/Private). Key Netskope configuration: (1) In Netskope portal (Policies > Profiles > Network Location > New Network Location), create 'MSFT SSE Service' profile with IPs: 150.171.19.0/24, 150.171.20.0/24, 13.107.232.0/24, 13.107.233.0/24, 150.171.15.0/24, 150.171.18.0/24, 151.206.0.0/16, 6.6.0.0/16. Create 'MSFT SSE M365' profile with Microsoft 365 IP ranges. (2) Create a Steering Configuration (Settings > Security Cloud Platform > Steering Configuration > New Configuration): add an exception for MSFT SSE Service with action Bypass + 'Treat it like local IP address', and add *.globalsecureaccess.microsoft.com as a domain exception. Enable the configuration and move it to the top. (3) If GSA handles Internet Access, also add a custom bypass in GSA Internet Access profile for *.goskope.com and Netskope IP ranges: 163.116.128.0/17, 162.10.0.0/17, 31.186.239.0/24, 8.39.144.0/24, 8.36.116.0/24. Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-netskope-coexistence",
  ...
  "Palo Alto Prisma / GlobalProtect": "Microsoft has published an official coexistence guide for Palo Alto Prisma Access / GlobalProtect and GSA (aka.ms/GSAPaloAlto). Coexistence is supported across 4 scenarios (GSA for Private/Internet/M365 + Prisma for Internet/Private). Key Prisma configuration: In Strata Cloud Manager, go to Workflows > Prisma Access Setup > GlobalProtect > GlobalProtect App > Tunnel Settings and add split-tunnel exclusions for *.globalsecureaccess.microsoft.com and GSA IPs: 150.171.19.0/24, 150.171.20.0/24, 13.107.232.0/24, 13.107.233.0/24, 150.171.15.0/24, 150.171.18.0/24, 151.206.0.0/16, 6.6.0.0/16. In App Settings, uncheck 'Resolve All FQDNs Using DNS Servers Assigned by the Tunnel' when using Microsoft Entra Private DNS (scenarios 1 and 2); enable it for scenarios 3 and 4. If GSA handles Internet Access, add a custom bypass for *.gpcloudservice.com in the GSA Internet Access profile (Entra admin center > Global Secure Access > Connect > Traffic forwarding > Internet access profile > Custom Bypass). Push config via Strata Cloud Manager > Manage > Operations > Push Status. Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-palo-alto-coexistence",
  "Cisco Secure Client / AnyConnect VPN": "Microsoft has published an official coexistence guide for Cisco VPNs and GSA (aka.ms/GSACiscoVPN). Use Split-Include mode only — full-tunnel AnyConnect/Secure Client and GSA cannot run simultaneously. Steps: (1) In the Cisco VPN profile, configure Split-Include to tunnel only private app subnets plus the GSA synthetic range 6.6.0.0/16. For Cisco Secure Access VPNaaS, add a GSA Internet Access bypass rule for *.vpn.sse.cisco.com. For Cisco ASA, add a bypass for the ASA endpoint FQDN and public IP. (2) Install Cisco Secure Client v5.1.10.x or later, then run as admin: '%ProgramFiles(x86)%\Cisco\Cisco Secure Client\acsocktool.exe' -slwm 10, followed by: net stop csc_vpnagent && net stop acsock && net start csc_vpnagent. (3) Validate using GSA Advanced Diagnostics (right-click tray icon > Advanced Diagnostics > Traffic tab). Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-cisco-vpn-coexistence",
  "Cisco Secure Client - Umbrella Module": "Microsoft has published an official coexistence guide for Cisco Umbrella and GSA (aka.ms/GSACiscoUmbrella). Coexistence is supported for DNS-only mode — Cisco SWG (Secure Web Gateway) must be disabled. Steps: (1) In GSA Internet Access profile (Entra admin center > Global Secure Access > Connect > Traffic forwarding > Internet access profile > Custom Bypass), add Umbrella IPs: 208.67.222.222, 208.67.220.220, 67.215.64.0/19, 146.112.0.0/16, 155.190.0.0/16, 185.60.84.0/22, 204.194.232.0/21, 208.67.216.0/21, 208.69.32.0/21. (2) In Umbrella portal, add *.globalsecureaccess.microsoft.com to Domain Management > Internal Domains, and add Microsoft 365 FQDNs (login.microsoftonline.com, outlook.office365.com, sharepoint.com, etc.) to the bypass list. If using Private Access, also add your Quick Access FQDN and Private DNS suffixes. (3) Install Cisco Secure Client v5.1.10.x+, then run as admin: '%ProgramFiles(x86)%\Cisco\Cisco Secure Client\acsocktool.exe' -slwm 10, then: net stop csc_vpnagent && net stop acsock && net start csc_vpnagent. (4) Restart Umbrella client services. Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-cisco-coexistence",
  "Cisco Umbrella Roaming Client": "Microsoft has published an official coexistence guide for Cisco Umbrella and GSA (aka.ms/GSACiscoUmbrella). Coexistence is supported with DNS-only Umbrella — SWG must be disabled. Steps: (1) In GSA Internet Access profile (Entra admin center > Global Secure Access > Connect > Traffic forwarding > Internet access profile > Custom Bypass), add Umbrella IPs: 208.67.222.222, 208.67.220.220, 67.215.64.0/19, 146.112.0.0/16, 155.190.0.0/16, 185.60.84.0/22, 204.194.232.0/21, 208.67.216.0/21, 208.69.32.0/21. (2) In the Umbrella portal, go to Deployments > Configuration > Domain Management > Internal Domains and add globalsecureaccess.microsoft.com (implied wildcard). Add Microsoft 365 FQDNs to the bypass list. If using Private Access, also add your Quick Access FQDN and Private DNS suffixes. (3) Restart Umbrella client services or restart the machine. Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-cisco-coexistence",
  ...
  "Cisco Secure Access": "Microsoft has published an official coexistence guide for Cisco Secure Access and GSA (aka.ms/GSACiscoSecureAccess). Multiple coexistence scenarios are supported depending on which product handles internet, private, and M365 traffic. Key steps for all scenarios: (1) In GSA Internet Access profile (Entra admin center > Global Secure Access > Connect > Traffic forwarding > Internet access profile > Custom Bypass), add Cisco Secure Access IPs (208.67.222.222, 208.67.220.220, 67.215.64.0/19, 146.112.0.0/16, 155.190.0.0/16, 185.60.84.0/22, 204.194.232.0/21, 208.67.216.0/21, 208.69.32.0/21) and FQDN *.zpc.sse.cisco.com. (2) In Cisco Secure Access portal, go to Connect > End User Connectivity > Internet Security > Traffic Steering, add *.globalsecureaccess.microsoft.com as Bypass Secure Access, and add GSA IP ranges as Bypass web proxy only: 150.171.19.0/24, 150.171.20.0/24, 13.107.232.0/24, 13.107.233.0/24, 6.6.0.0/16. Add Microsoft 365 FQDNs if Microsoft Access profile is enabled. (3) Install Cisco Secure Client v5.1.10.x+, then run as admin: '%ProgramFiles(x86)%\Cisco\Cisco Secure Client\acsocktool.exe' -slwm 10, then: net stop csc_vpnagent && net stop acsock && net start csc_vpnagent. Full guide: https://learn.microsoft.com/en-us/entra/global-secure-access/how-to-cisco-secure-access-coexistence",
